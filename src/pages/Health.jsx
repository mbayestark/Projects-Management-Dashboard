    ...
    <div className="mb-8">
        ...
        <div className="h-48 mb-4">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              <Line type="monotone" dataKey="weight" stroke="#E8A838" dot={false} strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
      <div className="flex gap-6 font-mono text-sm text-muted mb-4 flex-wrap">
        <span>Current <span className="text-text">{current ?? "—"} kg</span></span>
        <span>Lost <span className="text-text">{lost} kg</span></span>
        <span>Target <span className="text-text">{TARGET_WEIGHT} kg by {TARGET_DATE.slice(5)}</span></span>
      </div>
      <div className="flex gap-2 items-center">
        <input
          ...
          className="bg-card border border-border px-3 py-2 text-sm text-text outline-none w-32"
        />
        <button
          type="button"
          onClick={() => {
          ...
          className="px-4 py-2 bg-accent text-bg text-sm font-medium"
        >
          Log
        </button>
      </div>
...
function GymSection({ logs }) {
  ...
  const gymDates = new Set(
    logs.filter((l) => l.gymSession).map((l) => l.date)
  );
  ...
  for (const d of dates) {
    ...
    if (date >= startOfWeek && date < endOfWeek && gymDates.has(d)) {
      thisWeekSessions++;
    }
  }
  ...
  return (
    <div className="mb-8">
          ...
          <div key={week} className="flex flex-col gap-1.5">
            {Array.from({ length: 7 }, (_, day) => {
              ...
              return (
                <div
                  key={day}
                  className={`w-4 h-4 ${isFuture ? "bg-transparent" : gymDates.has(date) ? "bg-accent" : "bg-border"}`}
                  title={date}
                />
              );
            })}
      ...
      <div className="font-mono text-sm text-muted">
        This week: <span className="text-text">{thisWeekSessions} sessions</span>
      </div>
    </div>
  );
}
...
function HRSection({ logs }) {
  ...
  const chartData = logs
    .filter((l) => l.restingHR)
    .map((l) => ({ date: l.date.slice(5), hr: l.restingHR }));
  ...
  return (
    <div className="mb-8">
        ...
        <div className="h-40 mb-4">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              <Tooltip contentStyle={{ background: "#111", border: "1px solid #1F1F1F", color: "#F0EDE6" }} />
              <Line type="monotone" dataKey="hr" stroke="#E8A838" dot={false} strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
      <div className="font-mono text-sm text-muted mb-4">
        ...
        {delta !== null && (
          <span className="ml-4">
            Delta: <span className={delta < 0 ? "text-green-500" : "text-danger"}>{delta > 0 ? "+" : ""}{delta} bpm</span>
          </span>
        )}
      </div>
      <div className="flex gap-2 items-center">
      ...
      {showForm ? (
        <div className="border border-border p-3 flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setShowForm(false)} className="text-sm text-muted">
              Cancel
            </button>
          </div>
        </div>
